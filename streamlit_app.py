...
class DatabaseManager:
    ...
    def create_tables(self):
        ...
        if not cursor.fetchone():
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?)
            ''', ("Hamad", hashed_password, "admin@businesshelper.com",
    ...
    def verify_user(self, username, password):
        ...
        hashed_password = hashlib.sha256(password.encode()).hexdigest()
        cursor.execute(
            "SELECT id, username, email, package, full_name, country FROM users WHERE username = ? AND password = ? AND status = 'active'",
    ...
    def get_all_users(self):
        ...
        df = pd.read_sql_query(
            "SELECT id, username, email, full_name, country, package, created_date, status FROM users", conn
        )
        conn.close()
        return df
    ...
    def add_lead(self, company_name, industry, contact_person, email, phone, score):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (company_name, industry, contact_person, email, phone, score, 'active',
              datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
        conn.close()
    ...
    def save_email_campaign(self, campaign_name, subject, body, recipient_count):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (campaign_name, subject, body, recipient_count,
...
I hope this message finds you well. I'm reaching out because we've developed an AI-powered solution
that can help {company_name} increase leads by 300% and automate customer outreach.
...
def show_admin_dashboard():
    ...
    with col1:
        st.markdown(f"<div class='metric-container'><h3>👥</h3><h2>{stats['total_users']}</h2><p>Total Users</p></div>", unsafe_allow_html=True)
    with col2:
        st.markdown(f"<div class='metric-container'><h3>⭐</h3><h2>{stats['pro_users']}</h2><p>Pro Users</p></div>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"<div class='metric-container'><h3>🏆</h3><h2>{stats['business_users']}</h2><p>Business Users</p></div>", unsafe_allow_html=True)
    with col4:
        st.markdown(f"<div class='metric-container'><h3>📧</h3><h2>{stats['total_campaigns']}</h2><p>Email Campaigns</p></div>", unsafe_allow_html=True)

    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "👥 User Management", "🏢 Lead Management", "📊 Analytics", "💳 Revenue", "⚙️ Settings"
    ])
...
def show_user_dashboard():
    ...
    if selected == "Dashboard":
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.markdown("<div class='metric-container'><h2>50+</h2><p>Countries</p></div>", unsafe_allow_html=True)
        with col2:
            st.markdown("<div class='metric-container'><h2>10K+</h2><p>Jobs Available</p></div>", unsafe_allow_html=True)
        with col3:
            st.markdown("<div class='metric-container'><h2>5K+</h2><p>Business Leads</p></div>", unsafe_allow_html=True)
        with col4:
            st.markdown("<div class='metric-container'><h2>95%</h2><p>Success Rate</p></div>", unsafe_allow_html=True)
        ...
        if st.button("Generate Leads", type="primary"):
            ...
            for lead in leads:
                st.markdown(
                    ...
                    <div class='card'>
                        <h3>🏢 {lead['company']}</h3>
                        <p>👤 {lead['contact']} | 📧 {lead['email']} | 📞 {lead['phone']}</p>
        ...
        with st.form("email_form"):
            ...
            if st.form_submit_button("📤 Generate & Send Email", type="primary"):
                ...
                email_body = automator.generate_personalized_email(
                    template_map[template_type], company_name, recipient_name
                )
                if email_body:
        ...
        if st.button("🔄 Fetch Latest News", type="primary"):
            with st.spinner("Fetching news..."):
                ...
                if news:
                    ...
                    demo_news = [
                        "🚀 AI market grows 45% in Q1 2025",
                        "💰 Pakistan IT exports reach $3.5B",
                        "📈 Remote jobs increase 60% worldwide"
                    ]
        ...
        if st.button("Send", type="primary"):
            ...
            if "job" in user_lower:
                ...
                response = "Our packages: Free (5 emails/day), Starter (499 PKR), Pro (1499 PKR), Business (3999 PKR - Unlimited)"
            else:
